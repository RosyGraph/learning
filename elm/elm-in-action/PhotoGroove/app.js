...
var $author$project$PhotoGroove$viewThumbnail = F2(
	function (selectedUrl, thumb) {
		var urlPrefix = 'http://elm-in-action.com/';
		var classAttr = _Utils_eq(thumb.url, selectedUrl) ? 'selected' : 'thumbnail';
		return A2(
			$elm$html$Html$img,
			_List_fromArray(
				[
					$elm$html$Html$Attributes$src(
					_Utils_ap(urlPrefix, thumb.url)),
					$elm$html$Html$Attributes$class(classAttr)
				]),
			_List_Nil);
	});
			...
			[
				...
				_List_fromArray(
					[
						...
						$elm$html$Html$Attributes$src('http://elm-in-action.com/large/' + model.selectedUrl)
					]),
				_List_Nil)
			]));
};
